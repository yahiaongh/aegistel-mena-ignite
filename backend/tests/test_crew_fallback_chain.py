...
def test_clean_low_amount_with_request_qod_flag_does_not_provision(monkeypatch):
    # Regression: the "Auto-Provision QoD Slice on Risk" toggle must not force
    # a QoD session on a clean transaction. QoD is a consequence of risk or
    # high value only — otherwise an APPROVED verdict renders next to a
    # confusing "QoD REQUESTED" step-up.
    tool_names = []
    ...
    result = crew_specialists.run_specialist_crew(
        {"msisdn": "+99999991001", "amount": 100.0, "longitude": 46.7, "latitude": 24.7, "request_qod": True},
        [],
        [],
    )
